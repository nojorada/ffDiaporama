...
void cThumbnailComboBoxItem::paint(QPainter *painter,const QStyleOptionViewItem &option,const QModelIndex &index) const {
    ...
    if ((CurIndex>=0)&&(CurIndex<ComboBox->ModelTable->List.count())) {
        ...
        painter->drawImage(option.rect.left()+DECAL,option.rect.top()+DECAL,
                           ComboBox->ModelTable->List[CurIndex]->PrepareImage(0,ComboBox->Diaporama,ComboBox->ModelTable->List[CurIndex]->Name==QStringLiteral("*")?ComboBox->ProjectThumbnail:NULL));
...
void cThumbnailComboBox::PrepareTable(bool AllowCustomized,cModelList *Table) {
    ...
    if ((!AllowCustomized)&&(ModelTable->List.count()>0)&&(ModelTable->List[ModelTable->List.count()-1]->Name==QLatin1String("*"))) NbrItem--;
    int NbrRow=NbrItem/NBRCOLUMN;
    if (NbrRow*NBRCOLUMN<NbrItem) NbrRow++;

    for (int i=0;i<NbrRow;i++)
        addItem(QIcon(QPixmap().fromImage(ModelTable->List[i*NBRCOLUMN]->PrepareImage(0,Diaporama,ModelTable->List[i*NBRCOLUMN]->Name==QLatin1String("*")?ProjectThumbnail:NULL))),"");

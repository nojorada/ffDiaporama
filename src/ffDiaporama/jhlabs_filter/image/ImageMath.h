#ifndef __ImageMath__
#define __ImageMath__
/*
...
class ImageMath {
    ...
    /**
     * The value of pi as a qreal.
     */
	public:
      static qreal PI;

    /**
     * The value of half pi as a qreal.
     */
	static qreal HALF_PI;

    /**
     * The value of quarter pi as a qreal.
     */
	static qreal QUARTER_PI;

    /**
     * The value of two pi as a qreal.
     */
	static qreal TWO_PI;
	 ...
	 */
    public:
	   static qreal bias(qreal a, qreal b) 
      {
	 ...
	 * @return the output value
	 */
	static qreal gain(qreal a, qreal b) {
		qreal c = (1.0f/b-2.0f) * (1.0f-2.0f*a);
		if (a < 0.5)
			return a/(c+1.0f);
		else
			return (c-a)/(c-1.0f);
	 ...
	 */
	static qreal step(qreal a, qreal x) {
	 ...
	 * @return the output value - 0 or 1
	 */
	static qreal pulse(qreal a, qreal b, qreal x) {
	 ...
	 * @return the output value
	 */
	static qreal smoothPulse(qreal a1, qreal a2, qreal b1, qreal b2, qreal x) {
		if (x < a1 || x >= b2)
			return 0;
		if (x >= a2) {
			if (x < b1)
	 ...
	 * @return the output value
	 */
	static qreal smoothStep(qreal a, qreal b, qreal x) {
		if (x < a)
			return 0;
		if (x >= b)
			return 1;
	 ...
	 * @return the output value
	 */
	static qreal circleUp(qreal x) {
		x = 1-x;
		return (qreal)qSqrt(1-x*x);
	}
	 ...
	 * @return the output value
	 */
	static qreal circleDown(qreal x) {
		return 1.0f-(qreal)qSqrt(1-x*x);
	}
	 ...
	 */
	static qreal clamp(qreal x, qreal a, qreal b) {
	 ...
	 * @return a mod b
	 */
	 static qreal mod(qreal a, qreal b) {
		int n = (int)(a/b);
	 ...
	 */
	 static qreal lerp(qreal t, qreal a, qreal b) {
		return a + t * (b - a);
	}
	 ...
	 * @return the interpolated value
	 */
	 static int lerp(qreal t, int a, int b) {
		return (int)(a + t * (b - a));
	}
	 ...
	 * @return the interpolated value
	 */
	 static int mixColors(qreal t, int rgb1, int rgb2) {
		int a1 = (rgb1 >> 24) & 0xff;
		int r1 = (rgb1 >> 16) & 0xff;
		int g1 = (rgb1 >> 8) & 0xff;
		int b1 = rgb1 & 0xff;
	 ...
	 * @return the interpolated value
	 */
	static int bilinearInterpolate(qreal x, qreal y, int nw, int ne, int sw, int se) {
		qreal m0, m1;
		int a0 = (nw >> 24) & 0xff;
		int r0 = (nw >> 16) & 0xff;
		int g0 = (nw >> 8) & 0xff;
	 ...
	 */
	 /*
    static qreal spline(qreal x, int numKnots, qreal *knots) {
		int span;
	...
	*/
	 ...
	 */
	/*
   public static qreal spline(qreal x, int numKnots, int[] xknots, int[] yknots) {
		int span;
		int numSpans = numKnots - 3;
		qreal k0, k1, k2, k3;
		qreal c0, c1, c2, c3;
		...
		if (span > numKnots-3)
			span = numKnots-3;
		qreal t = (qreal)(x-xknots[span]) / (xknots[span+1]-xknots[span]);
		span--;
		if (span < 0) {
			span = 0;
			t = 0;
   ...
   */
	 ...
	 */
	 /*
    static int colorSpline(qreal x, int numKnots, int[] knots) {
		int span;
		int numSpans = numKnots - 3;
		qreal k0, k1, k2, k3;
		qreal c0, c1, c2, c3;
   ...
   */
	 ...
	 */
	 /*
    static void resample(int[] source, int[] dest, int length, int offset, int stride, qreal[] out) {
		int i, j;
		qreal sizfac;
		qreal inSegment;
		qreal outSegment;
		int a, r, g, b, nextA, nextR, nextG, nextB;
		qreal aSum, rSum, gSum, bSum;
		qreal[] in;
		int srcIndex = offset;
		...
		in = new qreal[length+2];
		i = 0;
		for (j = 0; j < length; j++) {
			while (out[i+1] < j)
				i++;
			in[j] = i + (qreal) (j - out[i]) / (out[i + 1] - out[i]);
//			in[j] = ImageMath.clamp( in[j], 0, length-1 );
		...
		while (i <= length) {
			qreal aIntensity = inSegment * a + (1.0f - inSegment) * nextA;
			qreal rIntensity = inSegment * r + (1.0f - inSegment) * nextR;
			qreal gIntensity = inSegment * g + (1.0f - inSegment) * nextG;
			qreal bIntensity = inSegment * b + (1.0f - inSegment) * nextB;
   ...
   */
	 ...
	 */
	/*
   static void premultiply( int[] p, int offset, int length ) {
        length += offset;
    ...
    */
	 ...
	 */
	/*
   static void unpremultiply( int[] p, int offset, int length ) {
        length += offset;
		for ( int i = offset; i < length; i ++ ) {
            int rgb = p[i];
            int a = (rgb >> 24) & 0xff;

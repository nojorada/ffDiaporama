 ...
 */
class GaussianFilter : public ConvolveFilter 
{
	 ...
	 */
	GaussianFilter(float radius)/*:kernel(3,3) */{
		setRadius(radius);
	}
   ...
   QImage *filter( const QImage &src, QImage *dst ) 
   {
        ...
        memcpy(inPixels,(const int *)src.bits(),src.QT_IMAGESIZE_IN_BYTES());
        ...
        memcpy(dst->bits(), inPixels,dst->QT_IMAGESIZE_IN_BYTES());
        delete [] inPixels;
        return dst;
    }
     ...
     */
	static void convolveAndTranspose(const Kernel &kernel, const int *inPixels, int *outPixels, int width, int height, bool alpha, bool premultiply, bool unpremultiply, int edgeAction) {
		float *matrix = kernel.getKernelData( 0 );
		int cols = kernel.getWidth();
		int cols2 = cols/2;
      //for(int i = 0; i < kernel.getHeight(); i++ )
      //   qDebug() << "matrix "<<i<< " is " <<matrix[i];
		for (int y = 0; y < height; y++) {
			...
			for (int x = 0; x < width; x++) {
               ...
               //qDebug()<< "faktor für " << moffset+col << " is " <<matrix[moffset+col];
					if (f != 0) {
						int ix = x+col;
						if ( ix < 0 ) {
     ...
     * @return the kernel
	 */
	static void makeKernel(Kernel *kernel, float radius) {
		...
		float sqrtSigmaPi2 = (float)qSqrt(sigmaPi2);

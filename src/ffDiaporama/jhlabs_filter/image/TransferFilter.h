#ifndef __TransferFilter__
#define __TransferFilter__
/*
...
class TransferFilter : public PointFilter 
{
	...
	int filterRGB(int x, int y, int rgb) 
   {
	...
	QImage *filter( const QImage &src, QImage *dst ) {
		if (!initialized)
			initialize();
		return PointFilter::filter( src, dst );
	}

...
class Noise : public Function1D, public Function2D, public Function3D 
{
	//private static Random randomGenerator = new Random();
	
	public:
   float evaluate(float x) { return noise1(x); }
	float evaluate(float x, float y) { return noise2(x, y); }
	float evaluate(float x, float y, float z) { return noise3(x, y, z); }
	...
	static float turbulence3(float x, float y, float z, float octaves) 
   {
	...
	static int p[B + B + 2];
	static float g3[B + B + 2][3];// = new float[B + B + 2][3];
	static float g2[B + B + 2][2];// = new float[B + B + 2][2];
	static float g1[B + B + 2];// = new float[B + B + 2];
	static bool start;
	...
	static float noise1(float x) 
   {
	...
	static void normalize2(float v[]) {
		float s = (float)qSqrt(v[0] * v[0] + v[1] * v[1]);
		v[0] = v[0] / s;
		v[1] = v[1] / s;
	}

	static void normalize3(float v[]) {
		float s = (float)qSqrt(v[0] * v[0] + v[1] * v[1] + v[2] * v[2]);
		v[0] = v[0] / s;
		v[1] = v[1] / s;
		v[2] = v[2] / s;
	}
	...
	private:
   static int random() {
		return /*randomGenerator.nextInt() */rand() & 0x7fffffff;
	}
...
class TurbulenceFunction : public CompoundFunction2D 
{
	float octaves;

	public:
      TurbulenceFunction(Function2D *basis, float octaves) : CompoundFunction2D(basis)
      {
      ...
      { 
         float t = 0.0f;
		   ...
		   for (float f = 1.0f; f <= octaves; f *= 2)
			   t += qFabs(basis->evaluate(f * x, f * y)) / f;
		   return t;
	   }
};
class MarbleFunction : public CompoundFunction2D 
{
	public:
   MarbleFunction() : CompoundFunction2D(new TurbulenceFunction(new Noise(), 6))
   {
		//super(new TurbulenceFunction(new Noise(), 6));
	}
	
	MarbleFunction(Function2D * basis) : CompoundFunction2D(basis) { }
	
	float evaluate(float x, float y) 
   {
		return (float)qPow(0.5 * (qSin(8. * basis->evaluate(x, y)) + 1), 0.77);
	}
};
#endif // __Noise__

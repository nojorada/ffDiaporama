   ...
   QCustomDialog(ApplicationConfig, parent), ui(new Ui::DlgImageCorrection)
{
   ...
   if (!BackgroundForm)
      BackgroundForm = &DefaultBackgroundForm;
...
void DlgImageCorrection::DoInitDialog()
{
   ...
   if (!GMapsWidget)
      ui->ExportMapBT->setVisible(false);
...
void DlgImageCorrection::DoPartialUndo()
{
   ...
   else
   {
      ...
      if (DomDocument.setContent(Data.Data, true))
      {
         ...
         {
            if (Data.FocusTab)
               ui->TabWidget->setCurrentIndex(ui->TabWidget->indexOf(Data.FocusTab));
...
void DlgImageCorrection::PreparePartialUndo(int /*ActionType*/, QDomElement root, bool /*DuplicateRessource*/)
{
   ...
   {
      if (CurrentBrush->MediaObject->is_Gmapsmap())
         CurrentBrush->MediaObject->SaveToXML(&root, "UNDO-DLG-OBJECT", "", true, NULL, NULL, false);
   }
   // special case for video object
   if (CurrentBrush->isVideo())
   {
      root.setAttribute("StartPos", ((cVideoFile *)CurrentBrush->MediaObject)->StartTime.toString("HH:mm:ss.zzz"));             // Start position (video only)
      root.setAttribute("EndPos", ((cVideoFile *)CurrentBrush->MediaObject)->EndTime.toString("HH:mm:ss.zzz"));                   // End position (video only)
...
void DlgImageCorrection::ApplyPartialUndo(int ActionType, QDomElement root)
{
   ...
   *BackgroundForm = root.attribute("BackgroundForm").toInt();
   ...
   if (CurrentBrush->isVideo())
   {
      ((cVideoFile *)CurrentBrush->MediaObject)->StartTime = QTime().fromString(root.attribute("StartPos"));
      ((cVideoFile *)CurrentBrush->MediaObject)->EndTime = QTime().fromString(root.attribute("EndPos"));
      CurrentBrush->SoundVolume = GetDoubleValue(root, "SoundVolume");
      CurrentBrush->Deinterlace = root.attribute("Deinterlace") == "1";
   }

   if ((BrushFileName != CurrentBrush->MediaObject->FileName()) && (ImageWidget))
      ImageWidget->ReloadFile(BrushFileName);
   RefreshControls();
   if ((GMapsWidget) && (ActionType == UNDOACTION_GMAPSMAPPART))
      GMapsWidget->ResetDisplayMap();
}

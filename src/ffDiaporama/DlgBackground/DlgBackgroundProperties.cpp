...
void DlgBackgroundProperties::DoInitDialog() {
    ...
    ui->BackgroundCombo->PrepareTable(DiaporamaObject->getDiaporama()->ImageGeometry,&BackgroundList);
...
void DlgBackgroundProperties::RefreshControls() 
{
   ...
   cBrushDefinition *BackgroundBrush = DiaporamaObject->BackgroundBrush;
   int BrushType = BackgroundBrush->BrushType;

   ui->KeepRatioRB->setChecked(!BackgroundBrush->FullFilling());
   ui->FullFillRB->setChecked(BackgroundBrush->FullFilling());
   ...
   if (ui->NewBackgroundRD->isChecked()) 
   {
      ...
      if (BackgroundList.SearchImage(BackgroundBrush->BrushImage)==-1 && BackgroundList.List.count()>0) 
         BackgroundBrush->BrushImage = BackgroundList.List[0].Name;
      ...
      ui->IntermPosED->setValue(BackgroundBrush->Intermediate*100);
      StopMAJSpinbox = false;

      bool FirstColorAllowed = (BrushType == BRUSHTYPE_SOLID || BrushType == BRUSHTYPE_PATTERN 
         || BrushType == BRUSHTYPE_GRADIENT2 || BrushType == BRUSHTYPE_GRADIENT3);
      ...
      ui->FinalColorCombo->setVisible(BrushType == BRUSHTYPE_GRADIENT2 || BrushType == BRUSHTYPE_GRADIENT3);
      ui->FinalColorCombo->setEnabled(BrushType == BRUSHTYPE_GRADIENT2 || BrushType == BRUSHTYPE_GRADIENT3);

      ui->IntermColorCombo->setVisible(BrushType == BRUSHTYPE_GRADIENT3);
      ui->IntermColorCombo->setEnabled(BrushType == BRUSHTYPE_GRADIENT3);

      ui->IntermPosSlider->setVisible(BrushType == BRUSHTYPE_GRADIENT3);
      ui->IntermPosSlider->setEnabled(BrushType == BRUSHTYPE_GRADIENT3);
      ui->IntermPosED->setVisible(BrushType == BRUSHTYPE_GRADIENT3);
      ui->IntermPosED->setEnabled(BrushType == BRUSHTYPE_GRADIENT3);

      ui->OrientationSpacer->setVisible(BrushType == BRUSHTYPE_GRADIENT2 || BrushType == BRUSHTYPE_GRADIENT3);
      ui->OrientationLabel->setVisible(BrushType == BRUSHTYPE_GRADIENT2 || BrushType == BRUSHTYPE_GRADIENT3);
      ui->OrientationCombo->setVisible(BrushType == BRUSHTYPE_GRADIENT2 || BrushType == BRUSHTYPE_GRADIENT3);
      ui->OrientationCombo->setEnabled(BrushType == BRUSHTYPE_GRADIENT2 || BrushType == BRUSHTYPE_GRADIENT3);

      ui->ImageLibraryLabel->setVisible(BrushType == BRUSHTYPE_IMAGELIBRARY);
      ui->ImageLibraryLabel2->setVisible(BrushType == BRUSHTYPE_IMAGELIBRARY);
      ui->BackgroundCombo->setVisible(BrushType == BRUSHTYPE_IMAGELIBRARY);
      ui->BackgroundComboSpacer->setVisible(BrushType == BRUSHTYPE_IMAGELIBRARY);

      ui->ImageFileLabel->setVisible(BrushType == BRUSHTYPE_IMAGEDISK);
      ui->ImageFileED->setVisible(BrushType == BRUSHTYPE_IMAGEDISK);
      ui->ImageFileBT->setVisible(BrushType == BRUSHTYPE_IMAGEDISK);
      ui->ImageEditCorrectBT->setVisible(BrushType == BRUSHTYPE_IMAGEDISK);
      ui->KeepRatioRB->setVisible(BrushType == BRUSHTYPE_IMAGEDISK);

      ui->ImageEditCorrectBT->setEnabled(BrushType == BRUSHTYPE_IMAGEDISK && !BackgroundBrush->FullFilling() && BackgroundBrush->MediaObject 
         && ( BackgroundBrush->MediaObject->is_Imagefile() 
           || BackgroundBrush->MediaObject->is_Imagevector()
           || BackgroundBrush->MediaObject->is_Imageclipboard()
           || BackgroundBrush->MediaObject->is_Gmapsmap()
           || BackgroundBrush->MediaObject->is_Videofile())
           );

      ui->FullFillRB->setVisible(BrushType == BRUSHTYPE_IMAGEDISK);
      ui->ImageFileED->setText(BackgroundBrush->MediaObject ? BackgroundBrush->MediaObject->FileName() : "");
   } 
   else 
   {
   ...
   int H = DiaporamaObject->getDiaporama()->GetHeightForWidth(W);
   if (H > ui->Preview->height()) 
   {
      H = ui->Preview->height();
      W = DiaporamaObject->getDiaporama()->GetWidthForHeight(H);
   }
   ...
   DiaporamaObject->getDiaporama()->PrepareBackground(DiaporamaObject->getDiaporama()->GetObjectIndex(DiaporamaObject),QSize(W,H),&Painter,(Background.width()-W)/2,(Background.height()-H)/2);
   Painter.end();
...
void DlgBackgroundProperties::s_SelectFile() {
    ...
    QString     NewFile="";
    DlgFileExplorer Dlg(BROWSER_TYPE_IMAGEONLY,false,false,false,QApplication::translate("DlgBackgroundProperties","Select a file"),DiaporamaObject->getDiaporama()->ApplicationConfig(),this);
    Dlg.InitDialog();
    if (Dlg.exec()==0) {
...
void DlgBackgroundProperties::s_ImageEditCorrect() {
    ...
    DlgImageCorrection Dlg(NULL,NULL,DiaporamaObject->BackgroundBrush,0,DiaporamaObject->getDiaporama()->ImageGeometry,SPEEDWAVE_DISABLE,ApplicationConfig,this);
    Dlg.InitDialog();
    if (Dlg.exec()==0) {
        RefreshControls();
    } else {
...
void DlgBackgroundProperties::s_FullFill() {
    ...
    DiaporamaObject->BackgroundBrush->setFullFilling(true);
    RefreshControls();
}
...
void DlgBackgroundProperties::s_KeepRatio() {
    ...
    DiaporamaObject->BackgroundBrush->setFullFilling(false);
    RefreshControls();
}

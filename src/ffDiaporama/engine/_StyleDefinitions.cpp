...
bool cStyleCollectionItem::LoadFromXML(QDomElement domDocument,QString ElementName,bool IsUserConfigFile,bool MustCheck) 
{
   if ((domDocument.elementsByTagName(ElementName).length() > 0) && (domDocument.elementsByTagName(ElementName).item(0).isElement() == true)) 
   {
      ...
      if ((MustCheck) && (Element.hasAttribute("BckStyleName")) && ((Element.attribute("BckStyleName") == "") || (Element.attribute("BckStyleName") != BckStyleName))) 
         return false;
...
#ifdef Q_OS_WIN
      if (StyleDef.endsWith("###FontName:Serif"))        
         StyleDef.replace("###FontName:Serif","###FontName:Times New Roman");
      if (StyleDef.endsWith("###FontName:Sans Serif"))   
         StyleDef.replace("###FontName:Sans Serif","###FontName:Arial");
#endif
...
{
    Collection.clear();
    for (int i = 0; i < UndoCollection->Collection.count(); i++) 
      Collection.append(new cStyleCollectionItem(UndoCollection->Collection[i]));
}
...
{
   QT_QSORT(Collection.begin(),Collection.end(),toAssending);
}
...
QString cStyleCollection::GetStyleName(QString StyleDef) 
{
   int i = 0;
   if (GeometryFilter) 
      while (( i < Collection.count()) && ((Collection[i].StyleDef != StyleDef) || (!Collection[i].StyleName.startsWith(ActiveFilter)))) 
         i++;
   else
      while (( i < Collection.count()) && (Collection[i].StyleDef != StyleDef)) 
         i++;

   if ((i < Collection.count()) && (Collection[i].StyleDef == StyleDef)) 
      return Collection[i].StyleName.mid(Collection[i].GetFilteredPart().length());
   else 
      return QApplication::translate("DlgManageStyle","Custom style");
}
...
QString cStyleCollection::GetStyleDef(QString StyleName) 
{
   ...
   {
      while (( i < Collection.count()) && (Collection[i].StyleName != StyleName)) 
         i++;
      if (( i < Collection.count()) && (Collection[i].StyleName == StyleName)) 
         return Collection[i].StyleDef; 
      else 
         return "";
   } 
   else 
   {
      while (( i < Collection.count()) && (Collection[i].StyleName != ActiveFilter + StyleName)) 
         i++;
      if (( i < Collection.count()) && (Collection[i].StyleName == ActiveFilter+StyleName)) 
         return Collection[i].StyleDef; 
      else 
         return "";
   }
        ...
        while ((Element.elementsByTagName(CollectionName+QString("Item_%1").arg(i)).length()>0)&&(root.elementsByTagName(CollectionName+QString("Item_%1").arg(i)).item(0).isElement()==true)) 
        {
            ...
            else 
            {
                ...
                if ((root.elementsByTagName(ElementName).length()>0)&&(root.elementsByTagName(ElementName).item(0).isElement()==true)) 
                {
                    ...
                    int j=0;
                    while ((j<Collection.count())&&(Collection[j].StyleIndex!=IndexKey)) 
                       j++;
...
void cStyleCollection::FillCollectionCB(QComboBox *CB,QString ActualStyleName) 
{
    ...
    for (int i=0;i<Collection.count();i++) 
    {
        ...
        if (ActualStyleName==Collection[i].StyleName) 
           CB->setCurrentIndex(i);
    }

    //if (CB->view()->width()!=500)  CB->view()->setFixedWidth(500);
    if (!CB->updatesEnabled()) 
...
QString cStyleCollection::PopupCollectionMenu(QWidget *ParentWindow,cApplicationConfig *BaseApplicationConfig,QString ActualStyleDef) 
{
   ...
   ContextMenu->setFont(popupFont);
   ActionCreate->setFont(popupFont);
   ActionManage->setFont(popupFont);
   UpdateMenu->setTitle(QApplication::translate("DlgManageStyle", "Update existing style"));
   UpdateMenu->setFont(popupFont);
   ...
   for (int i = 0; i < Collection.count(); i++)
   {
         ...
         )
      {
         ...
         NormalAction->setFont(popupFont);
         ...
         UpdateAction->setFont(popupFont);
   ...
   {
      if (Ret == ActionCreate)                                                              
         CreateNewStyle(ParentWindow, ActualStyleDef);
      else if (Ret == ActionManage)                                                         
         ManageExistingStyle(ParentWindow, BaseApplicationConfig);
      else if (Ret->toolTip() == QApplication::translate("DlgManageStyle", "Update this style"))  
         UpdateExistingStyle((GeometryFilter ? ActiveFilter : "") + Ret->text(), ActualStyleDef);
      else 
         Item = /*(GeometryFilter?ActiveFilter:"")+*/ Ret->text();
   }
   delete ContextMenu;
   delete UpdateMenu;
   delete ActionCreate;
...
{
   if (!String.contains("###GLOBALSTYLE###:")) 
      return String;
   int StyleIndex = String.mid(QString("###GLOBALSTYLE###:").length()).toInt();
   int i = 0;
   while (( i < Collection.count()) && (Collection[i].StyleIndex != StyleIndex)) 
      i++;
   if (( i < Collection.count()) && (Collection[i].StyleIndex == StyleIndex)) 
   {
      QString Filter = Collection[i].GetFilteredPart();
      if (Filter == "") 
         return Collection[i].StyleName; 
      else 
         return Collection[i].StyleName.mid(Filter.length());
   } 
   return "";
}

...
void DlgGMapsGeneration::DoRejet() {
    ...
    if (DuplicateRessource)
        MediaObject->setRessourceKey(-1);
    MediaObject->setSlideThumb(DestMap);
...
void DlgGMapsGeneration::DoInitDialog() 
{
   ...
   for (int i = 0; i < MediaObject->locations.count(); i++) 
      if (MediaObject->locations[i]->LocationType != cLocation::FREE) 
      {
         cLocation *Location = MediaObject->locations[i];
      ...
      if (!MediaObject->IsMapValide) 
         MediaObject->RequestList.clear();
      if (MediaObject->RequestList.isEmpty()) 
      {
         ...
         MediaObject->getSlideThumb(&DestMap); 
      //ApplicationConfig->SlideThumbsTable->GetThumbs(&MediaObject->RessourceKey,&DestMap);

...
DlgApplicationSettings::DlgApplicationSettings(cApplicationConfig *ApplicationConfig,QWidget *parent)
    ...
    needReInitLibav = false;
    CurrentDevice       =-1;
    CurrentDeviceIndex  =-1;

    bgCUDA = new QButtonGroup(this);
    bgCUDA->addButton(ui->rbHWAccAutodetect);
    bgCUDA->addButton(ui->rbDisablHWAccDetect);

    bgQSVEnc = new QButtonGroup(this);
    bgQSVEnc->addButton(ui->rbQSVEncAutodetect);
    bgQSVEnc->addButton(ui->rbQSVEncDisabled);

    bgCUVID = new QButtonGroup(this);
    bgCUVID->addButton(ui->rbCUVIDAutodetect);
    bgCUVID->addButton(ui->rbCUVIDDsiabled);
}
...
void DlgApplicationSettings::DoInitDialog() {
    ...
    ui->DisableTooltipsCB->setChecked(ApplicationConfig->DisableTooltips);
        ...
        else if (ApplicationConfig->MemCacheMaxValue<=int64_t(2048ll*1024ll*1024ll))     ui->MemCacheProfilCB->setCurrentIndex(3);
        else ui->MemCacheProfilCB->setCurrentIndex(1);
        ui->MemCacheProfilCB->removeItem(4);
        ui->MemCacheProfilCB->removeItem(4);
        ui->MemCacheProfilCB->removeItem(4);
        ui->MemCacheProfilCB->removeItem(4);
        ui->MemCacheProfilCB->removeItem(4);
        ...
        else if (ApplicationConfig->MemCacheMaxValue<=int64_t(3072ll*1024ll*1024ll))    ui->MemCacheProfilCB->setCurrentIndex(4);
        else if (ApplicationConfig->MemCacheMaxValue<=int64_t(4096ll*1024ll*1024ll))    ui->MemCacheProfilCB->setCurrentIndex(5);
        else if (ApplicationConfig->MemCacheMaxValue<=int64_t(5120ll*1024ll*1024ll))    ui->MemCacheProfilCB->setCurrentIndex(6);
        else ui->MemCacheProfilCB->setCurrentIndex(7);
    #endif
    ...
    ui->rbHWAccAutodetect->setChecked(ApplicationConfig->enableCUDA);
    ui->rbDisablHWAccDetect->setChecked(!ApplicationConfig->enableCUDA);

    ui->rbQSVEncAutodetect->setChecked(ApplicationConfig->enableQSVEnc);
    ui->rbQSVEncDisabled->setChecked(!ApplicationConfig->enableQSVEnc);

    ui->rbCUVIDAutodetect->setChecked(ApplicationConfig->enableCUVID);
    ui->rbCUVIDDsiabled->setChecked(!ApplicationConfig->enableCUVID);
#else
    ui->lblUseCuda->hide();
    ui->rbHWAccAutodetect->hide();
    ui->rbDisablHWAccDetect->hide();
    ui->cbPreferrdCUDA->hide();
    ui->hsCudaEncoder->changeSize(0,0);
    
    ui->lbUseQSVEncoder->hide();
    ui->rbQSVEncAutodetect->hide();
    ui->rbQSVEncDisabled->hide();
    ui->cbPreferredQSV->hide();
    ui->hsQSVEncoder->changeSize(0, 0);

    ui->lbUseCUVID->hide();
    ui->rbCUVIDAutodetect->hide();
    ui->rbCUVIDDsiabled->hide();
    ui->cbPreferredCUVID->hide();
    ui->hsCuvidDecoder->changeSize(0, 0);
#endif
    ui->cbEnableYUVPassthrough->setChecked(ApplicationConfig->enableYUVPassthrough);
    ui->cbEnableSmoothSVG->setChecked(ApplicationConfig->enableSmoothSVG);
    ui->cbEnableCRF->setChecked(ApplicationConfig->enableCRF);
    ...
    ui->NoTransitionRD->setChecked((!ApplicationConfig->RandomTransition)&&(ApplicationConfig->DefaultTransitionFamily==0)&&(ApplicationConfig->DefaultTransitionSubType==0));
    ui->BasicTransitionB1RD->setChecked((!ApplicationConfig->RandomTransition)&&(ApplicationConfig->DefaultTransitionFamily==0)&&(ApplicationConfig->DefaultTransitionSubType==1));
    ui->BasicTransitionB2RD->setChecked((!ApplicationConfig->RandomTransition)&&(ApplicationConfig->DefaultTransitionFamily==0)&&(ApplicationConfig->DefaultTransitionSubType==2));
    ...
    if( ui->SmartphoneTypeCB->count() > 0 )
    ...
    for (int i = 0; i < List.count(); i++)
    {
       ...
       if (ApplicationConfig->DefaultMultimediaType == ItemData)
          ui->MMSystemTypeCB->setCurrentIndex(i);
    }
    connect(ui->MMSystemTypeCB,SIGNAL(currentIndexChanged(int)),this,SLOT(ChangeMMSystemTypeCB(int)));
    if( ui->MMSystemTypeCB->count() > 0 )
...
void DlgApplicationSettings::AddItemToFramingCB(QComboBox *CB,int FraminStyle) 
{
    ...
    case AUTOFRAMING_GEOMETRY_CUSTOM:
       IconGeoImage = FraminStyle == AUTOFRAMING_CUSTOMUNLOCK ? QIcon(AUTOFRAMING_ICON_GEOMETRY_UNLOCKED) : QIcon(AUTOFRAMING_ICON_GEOMETRY_LOCKED);
       break;
    case AUTOFRAMING_GEOMETRY_PROJECT:
       IconGeoImage = QIcon(AUTOFRAMING_ICON_GEOMETRY_PROJECT);
       break;
    case AUTOFRAMING_GEOMETRY_IMAGE:
       IconGeoImage = QIcon(AUTOFRAMING_ICON_GEOMETRY_IMAGE);
       break;
    }
...
bool DlgApplicationSettings::DoAccept() 
{
    ...
    {
        case 7  : ApplicationConfig->MemCacheMaxValue=int64_t(6144*int64_t(1024*1024));    break;
        case 6  : ApplicationConfig->MemCacheMaxValue=int64_t(5120*int64_t(1024*1024));    break;
        case 5  : ApplicationConfig->MemCacheMaxValue=int64_t(4096*int64_t(1024*1024));    break;
        case 4  : ApplicationConfig->MemCacheMaxValue=int64_t(3072*int64_t(1024*1024));    break;
...
#ifdef ENABLE_FFMPEG_CUDA
    if (ApplicationConfig->enableCUDA != ui->rbHWAccAutodetect->isChecked())
       needReInitLibav = true;
    if (ApplicationConfig->enableQSVEnc != ui->rbQSVEncAutodetect->isChecked())
       needReInitLibav = true;
    ApplicationConfig->enableCUDA = ui->rbHWAccAutodetect->isChecked();
    ApplicationConfig->enableQSVEnc = ui->rbQSVEncAutodetect->isChecked();
    ApplicationConfig->enableCUVID = ui->rbCUVIDAutodetect->isChecked();
#endif
    ApplicationConfig->enableYUVPassthrough = ui->cbEnableYUVPassthrough->isChecked();
    ApplicationConfig->enableSmoothSVG = ui->cbEnableSmoothSVG->isChecked();
    if (ApplicationConfig->enableCRF != ui->cbEnableCRF->isChecked())
       needReInitLibav = true;
    ApplicationConfig->enableCRF = ui->cbEnableCRF->isChecked();
    ...
    ApplicationConfig->DefaultTransitionFamily      = TRANSITIONFAMILY_BASE;
    ...
    if (Codec >= 0)
       ApplicationConfig->DefaultVideoCodec = VIDEOCODECDEF[ui->VideoFormatCB->itemData(Codec).toInt()].FFD_VCODECST; else ApplicationConfig->DefaultVideoCodec = "";
    Codec=ui->AudioFormatCB->currentIndex();
    if (Codec >= 0)
       ApplicationConfig->DefaultAudioCodec = AUDIOCODECDEF[ui->AudioFormatCB->itemData(Codec).toInt()].ShortName; else ApplicationConfig->DefaultAudioCodec = "";
    QString BitRate = ui->VideoBitRateCB->currentText();
    if (BitRate.endsWith("k"))
       BitRate = BitRate.left(BitRate.length() - 1);
    ApplicationConfig->DefaultVideoBitRate = BitRate.toInt();
    ApplicationConfig->DefaultVideoCRF = ui->sbVideoCRF->value();
    BitRate = ui->AudioBitRateCB->currentText();
    if (BitRate.endsWith("k"))
       BitRate = BitRate.left(BitRate.length() - 1);
    ApplicationConfig->DefaultAudioBitRate = BitRate.toInt();
    QString Freq = ui->AudioFreqCB->currentText();
    ApplicationConfig->DefaultFreq = Freq.toInt();

    ApplicationConfig->DefaultSoundtrackFormat = ui->SoundtrackFileFormatCB->currentIndex();
    if (ApplicationConfig->DefaultSoundtrackFormat >= 0)
       ApplicationConfig->DefaultSoundtrackFormat = ui->SoundtrackFileFormatCB->itemData(ApplicationConfig->DefaultSoundtrackFormat).toInt();
    else
       ApplicationConfig->DefaultSoundtrackFormat = 0;
    Codec = ui->SoundtrackAudioFormatCB->currentIndex();
    if (Codec >= 0)
       ApplicationConfig->DefaultSoundtrackAudioCodec = AUDIOCODECDEF[ui->SoundtrackAudioFormatCB->itemData(Codec).toInt()].ShortName;
    else
       ApplicationConfig->DefaultSoundtrackAudioCodec = "";
    BitRate = ui->SoundtrackAudioBitRateCB->currentText();
    if (BitRate.endsWith("k"))
       BitRate = BitRate.left(BitRate.length() - 1);
    ApplicationConfig->DefaultSoundtrackBitRate = BitRate.toInt();
    Freq = ui->SoundtrackAudioFreqCB->currentText();
    ApplicationConfig->DefaultSoundtrackFreq = Freq.toInt();
    ...
    cDeviceModelDef *device;
    device = ApplicationConfig->DeviceModelList.getDevice(Text);
    if (device)
       ApplicationConfig->DefaultSmartphoneModel = device->DeviceIndex;

    Text = ui->MMSystemModelCB->currentText();
    device = ApplicationConfig->DeviceModelList.getDevice(Text);
    if (device)
       ApplicationConfig->DefaultMultimediaModel = device->DeviceIndex;
    Text=ui->ForTheWModelCB->currentText();
    device = ApplicationConfig->DeviceModelList.getDevice(Text);
    if (device)
       ApplicationConfig->DefaultForTheWEBModel = device->DeviceIndex;
    ...
    if (needReInitLibav)
       ApplicationConfig->DeviceModelList.reInitLibav();
    return true;
}
...
void DlgApplicationSettings::InitImageSizeCombo(int) 
{
   ...
   for (int i = 0; i < List.count(); i++)
   {
      ...
      {
         QString StrA = List[j].mid(List[j].lastIndexOf(":") + 1);
         StrA = StrA.left(StrA.indexOf("#"));
         QString StrB = List[j + 1].mid(List[j + 1].lastIndexOf(":") + 1);
         StrB = StrB.left(StrB.indexOf("#"));
...
void DlgApplicationSettings::ChangeSmartphoneTypeCB(int) 
{
   ...
   if (ItemData >= 0) 
      ItemData = ui->SmartphoneTypeCB->itemData(ItemData).toInt();
   QStringList List;
   ...
   int i = 0;
   while ((i < ApplicationConfig->DeviceModelList.RenderDeviceModel.count()) && (ApplicationConfig->DeviceModelList.RenderDeviceModel[i]->DeviceIndex != ApplicationConfig->DefaultSmartphoneModel)) 
      i++;
   if ( (i < ApplicationConfig->DeviceModelList.RenderDeviceModel.count()) && 
      ApplicationConfig->DeviceModelList.RenderDeviceModel[i]->DeviceIndex == ApplicationConfig->DefaultSmartphoneModel)
...
void DlgApplicationSettings::ChangeMMSystemTypeCB(int) 
{
   ...
   int i = 0;
   while ((i < ApplicationConfig->DeviceModelList.RenderDeviceModel.count()) && (ApplicationConfig->DeviceModelList.RenderDeviceModel[i]->DeviceIndex != ApplicationConfig->DefaultMultimediaModel)) 
      i++;
   if ((i < ApplicationConfig->DeviceModelList.RenderDeviceModel.count()) && 
      ApplicationConfig->DeviceModelList.RenderDeviceModel[i]->DeviceIndex == ApplicationConfig->DefaultMultimediaModel)
...
void DlgApplicationSettings::ChangeForTheWTypeCB(int) 
{
   ...
   if ((i < ApplicationConfig->DeviceModelList.RenderDeviceModel.count()) && ApplicationConfig->DeviceModelList.RenderDeviceModel[i]->DeviceIndex == ApplicationConfig->DefaultForTheWEBModel)
...
void DlgApplicationSettings::InitVideoBitRateCB(int ChangeIndex)
{
   ...
   if (CurrentCodec >= 0)
   {
      ...
      bool useCRF = ApplicationConfig->enableCRF && VIDEOCODECDEF[CurrentCodec].hasCRF;
      if (useCRF)
      {
         ui->lVideoBitrate->setText("CRF :");
         ui->VideoBitRateCB->hide();
         ui->sbVideoCRF->show();
         ui->crfSpacer->changeSize(10, 10,QSizePolicy::Expanding);
         ui->horizontalLayout_22->invalidate();
         ui->sbVideoCRF->setMaximum(VIDEOCODECDEF[CurrentCodec].maxCRF);
         ui->sbVideoCRF->setMinimum(VIDEOCODECDEF[CurrentCodec].minCRF);
         ui->sbVideoCRF->setValue(ApplicationConfig->DefaultVideoCRF/*VIDEOCODECDEF[CurrentCodec].defaultCRF*/);
         //ApplicationConfig->DefaultVideoCRF;

      }
      else
      {
         ui->lVideoBitrate->setText("Bit rate :");
         ui->VideoBitRateCB->show();
         ui->sbVideoCRF->hide();
         ui->crfSpacer->changeSize(0, 0);
         ...
         if (!IsFindBitRate)
            ui->VideoBitRateCB->setCurrentIndex(ui->VideoBitRateCB->findText(VIDEOCODECDEF[CurrentCodec].DefaultBitrate[Standard][CurrentSize]));

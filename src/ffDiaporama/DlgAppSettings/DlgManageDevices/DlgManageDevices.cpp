...
void DlgManageDevices::DoInitDialog() 
{
   ...
   for (int i = 0; i < VFORMAT_NBR; i++)
      if (FORMATDEF[i].IsFind)
         ui->DBFileFormatCB->addItem(FORMATDEF[i].LongName, QVariant(i));
   ...
   ui->sbCRF->hide();
   connect(ui->sbCRF, SIGNAL(valueChanged(int)), this, SLOT(DBChCRFsb(int)));
}
...
void DlgManageDevices::switchToCRF(bool hasCRF, cDeviceModelDef *Device)
{
   if (hasCRF)
   {
      ui->VideoBitRateLabel->setText("CRF:");
      ui->sbCRF->show();
      ui->sbCRF->setMaximum(VIDEOCODECDEF[Device->VideoCodec].maxCRF);
      ui->sbCRF->setMinimum(VIDEOCODECDEF[Device->VideoCodec].minCRF);
      if( Device->VideoCRF == 0)
          Device->VideoCRF = VIDEOCODECDEF[Device->VideoCodec].defaultCRF;
      ui->sbCRF->setValue(Device->VideoCRF);
      ui->DBVideoBitRateCB->hide();
   }
   else
   {
      ui->sbCRF->hide();
      ui->DBVideoBitRateCB->show();
      ui->VideoBitRateLabel->setText(QApplication::translate("DlgManageDevices", "Bit rate :", nullptr));
...
void DlgManageDevices::DBFillTableDevice(int ChangeIndex) 
{
   ...
   for (int i = 0; i < DeviceModelList->RenderDeviceModel.count(); i++)
      if (DeviceModelList->RenderDeviceModel[i]->DeviceType == ChangeIndex + 1)
      {
...
void DlgManageDevices::DBSelectionChanged() 
{
   ...
   else
   {
      ...
      if (DeviceNumber.startsWith("#")) 
         DeviceNumber = DeviceNumber.mid(1);
      CurrentDeviceIndex = DeviceNumber.toInt();
      CurrentDevice = 0; 
      while (DeviceModelList->RenderDeviceModel[CurrentDevice]->DeviceIndex != CurrentDeviceIndex) 
         CurrentDevice++;
      ...
      QString VideoBitRateStr = QString("%1").arg(Device->VideoBitrate); 
      if (VideoBitRateStr.endsWith("000")) 
         VideoBitRateStr = VideoBitRateStr.left(VideoBitRateStr.length() - 3) + "k";
      QString AudioBitRateStr = QString("%1").arg(Device->AudioBitrate);
      if (AudioBitRateStr.endsWith("000")) 
         AudioBitRateStr = AudioBitRateStr.left(AudioBitRateStr.length() - 3) + "k";
      else 
         AudioBitRateStr = QString("%1.%2k").arg(Device->AudioBitrate / 1000).arg((Device->AudioBitrate % 1000) / 10);

      switchToCRF(VIDEOCODECDEF[Device->VideoCodec].hasCRF && ApplicationConfig->enableCRF, Device);
      ui->DBVideoBitRateCB->setCurrentIndex(ui->DBVideoBitRateCB->findText(VideoBitRateStr));
...
void DlgManageDevices::DBImageSizeCombo(int) 
{
   ...
   for (int i = 0; i < NBR_SIZEDEF; i++)
      if (ORDERIMAGENAME[Standard][i] != 0)
         List.append(QString("%1:%2#####%3").arg(IMAGEDEFGENNAME[Standard][i]).arg(ORDERIMAGENAME[Standard][i]).arg(i));
   // Sort List
   for (int i = 0; i < List.count(); i++)
...
void DlgManageDevices::DBChCRFsb(int /*value*/)
{
   IsDeviceChanged = true;
   ui->DBApplyBT->setEnabled(IsDeviceChanged);
}
...
void DlgManageDevices::DBVideoBitRateCB(int) 
{
   ...
   if (CurrentDevice >= 0)
   {
      ...
      while (DeviceModelList->RenderDeviceModel[CurrentDevice]->DeviceIndex != CurrentDeviceIndex)
         CurrentDevice++;
      Device = DeviceModelList->RenderDeviceModel[CurrentDevice];
   }
   ...
   if (CurrentCodec >= 0)
   {
      ...
      if (Device && VIDEOCODECDEF[CurrentCodec].hasCRF&& ApplicationConfig->enableCRF)
      {
         Device->VideoCodec = CurrentCodec;
         if (Device->VideoCRF < 0 )
            Device->VideoCRF = VIDEOCODECDEF[CurrentCodec].defaultCRF;
         switchToCRF(VIDEOCODECDEF[CurrentCodec].hasCRF && ApplicationConfig->enableCRF, Device);
      }
      ...
      for (int i = 0; i < List.count(); i++)
         for (int j = 0; j < List.count() - 1; j++)
         {
            QString NameA = List[j];
            if (NameA.endsWith("k"))
               NameA = NameA.left(NameA.length() - 1);
            int NumA = NameA.toInt();
            QString NameB = List[j + 1];
            if (NameB.endsWith("k"))
               NameB = NameB.left(NameB.length() - 1);
      ...
      ui->DBVideoBitRateCB->setEnabled(false);
   IsDeviceChanged = true;
   ui->DBApplyBT->setEnabled(IsDeviceChanged);
}
...
void DlgManageDevices::DBAudioBitRateCB(int) 
{
   ...
   if (CurrentCodec >= 0)
   {
      ...
      ui->DBAudioBitRateCB->setEnabled(false);
   IsDeviceChanged = true;
   ui->DBApplyBT->setEnabled(IsDeviceChanged);
}
...
void DlgManageDevices::DBResetToDefaultBT() 
{
   ...
   if (CurrentDevice >= 0)
   {
      ...
      Device->VideoCRF = Device->BckVideoCRF;
...
void DlgManageDevices::DBApplyChange() 
{
   ...
   if (CurrentDevice >= 0)
   {
      ...
      QString VideoBitRateStr = ui->DBVideoBitRateCB->currentText(); 
      if (VideoBitRateStr.endsWith("k")) 
         VideoBitRateStr = VideoBitRateStr.left(VideoBitRateStr.length() - 1) + "000";
      QString AudioBitRateStr = ui->DBAudioBitRateCB->currentText(); 
      if (AudioBitRateStr.endsWith("k")) 
         AudioBitRateStr = AudioBitRateStr.left(AudioBitRateStr.length() - 1) + "000";
      Device->VideoBitrate = VideoBitRateStr.toInt();
      Device->VideoCRF = ui->sbCRF->value();
...
void DlgManageDevices::DBAddDevice() 
{
   ...
   int DeviceIndex = 0;
   for (int i = 0; i<DeviceModelList->RenderDeviceModel.count(); i++) 
      if (DeviceModelList->RenderDeviceModel[i]->DeviceIndex>DeviceIndex) 
         DeviceIndex = DeviceModelList->RenderDeviceModel[i]->DeviceIndex;
   DeviceIndex++;
...
void DlgManageDevices::DBRemoveBT() 
{
   ...
   if ((CurrentDevice < 0) || (CurrentDevice >= DeviceModelList->RenderDeviceModel.count())) 
      return;
   if (DeviceModelList->RenderDeviceModel[CurrentDevice]->FromGlobalConf) 
      return;

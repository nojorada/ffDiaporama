...
{
   if ((Window == NULL) || (*RestoreWindow == false) || (!IsInit)) 
      return;
...
{
   if ((Window == NULL) || (*RestoreWindow == false)) 
      return;
...
void cSaveWindowPosition::LoadFromXML(QDomElement domDocument) 
{
   if ((domDocument.elementsByTagName(WindowName).length() > 0) && (domDocument.elementsByTagName(WindowName).item(0).isElement() == true))
   {
      ...
      if (IsMainWindow && (Element.hasAttribute("IsMaximized")))  IsMaximized = Element.attribute("IsMaximized") == QLatin1String("1");
...
void cSaveWinWithSplitterPos::ApplyToWindow(QWidget *Window,QSplitter *Top) 
{
   ...
   if (SplitterTop != "") 
      Top->restoreState(QByteArray::fromHex(SplitterTop.toUtf8()));
}
...
{
   if (Element.hasAttribute("SplitterTop"))
      SplitterTop = Element.attribute("SplitterTop");
}

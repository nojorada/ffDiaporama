...
QList<QPolygonF> ComputePolygonR(double width,double height,double CenterX,double CenterY,int MaxPoint,double StartAngle) 
{
   ...
   {
      vcos = cos(Angle * PI/180) * (width/2);
      vsin = sin(Angle * PI/180) * (height/2);
      Polygon << QPointF(CenterX+vcos,CenterY-vsin);
      Angle = Angle + (double(360.0)/MaxPoint);
      if (Angle >= 360) 
         Angle = -Angle+360;
   }
...
QList<QPolygonF> ComputeDoubleArrow(QRectF Rect,int Angle) 
{
   ...
   for (int i = 1; i < 11; i++) 
      Path.lineTo(QPointF(Table[i].x()*ScaleX,Table[i].y()*ScaleY)+QPointF(Rect.left(),Rect.top()));
...
{
   //qDebug() << "ComputeStar "<< Pointed;
   QList<QPolygonF> polygonList;
   //QPointF Table[100];
   QPainterPath Path;
   //QPainterPath starPath;
   QPolygonF star;
   qreal Segment = (180/Pointed);
   qreal CurAngle = -90;
   ...
   for (int i = 0; i <= Pointed; i++) 
   {
      ...
      star.append(QPointF(cos(CurAngle*PI/180)*Ray1+Ray1, sin(CurAngle*PI/180)*Ray1+Ray1));
      CurAngle += Segment;
      //Table[i*2+1]=QPointF(cos(CurAngle*PI/180)*Ray2+Ray1,sin(CurAngle*PI/180)*Ray2+Ray1);
      star.append(QPointF(cos(CurAngle*PI/180)*Ray2+Ray1, sin(CurAngle*PI/180)*Ray2+Ray1));
      CurAngle += Segment;
   }
   ...
   //qreal ScaleX=Rect.width()/(x2-x1);
   //qreal ScaleY=Rect.height()/(y2-y1);
   qreal ScaleX = Rect.width()/star.boundingRect().width();
   qreal ScaleY = Rect.height()/star.boundingRect().height();
   star = QTransform::fromScale(ScaleX, ScaleY).map(star);
   star = QTransform::fromTranslate(Rect.left()-star.boundingRect().left(), Rect.top()-star.boundingRect().top()).map(star);

   //Path.moveTo(QPointF(Table[0].x()*ScaleX,Table[0].y()*ScaleY)+QPointF(Rect.left(),Rect.top()));
   //qDebug() << QPointF(Table[0].x()*ScaleX,Table[0].y()*ScaleY)+QPointF(Rect.left(),Rect.top()) << " " << star.at(0);
   //for (int i=1;i<Pointed*2;i++) 
   //{
   //   Path.lineTo(QPointF(Table[i].x()*ScaleX,Table[i].y()*ScaleY)+QPointF(Rect.left(),Rect.top()));
   //   qDebug() << QPointF(Table[i].x()*ScaleX,Table[i].y()*ScaleY)+QPointF(Rect.left(),Rect.top()) << " " << star.at(i);
   //}
   Path.moveTo(star.at(0));
   for (int i = 1; i < Pointed*2; i++) 
      Path.lineTo(star.at(i));

   polygonList.append(Path.toFillPolygon(QTransform()));
   return polygonList;
}
...
QList<QPolygonF> ComputeGear(QRectF Rect,int tooth) 
{
   ...
   {
      gear.append(QPointF(cos(CurAngle*pi180)*Ray1+Ray1,sin(CurAngle*pi180)*Ray1+Ray1));
      CurAngle += PartSegment;
      gear.append(QPointF(cos(CurAngle*pi180)*Ray1+Ray1,sin(CurAngle*pi180)*Ray1+Ray1));
      CurAngle += Segment-PartSegment;
      gear.append(QPointF(cos(CurAngle*pi180)*Ray2+Ray1,sin(CurAngle*pi180)*Ray2+Ray1));
      CurAngle += PartSegment;
      gear.append(QPointF(cos(CurAngle*pi180)*Ray2+Ray1,sin(CurAngle*pi180)*Ray2+Ray1));
      CurAngle += Segment-PartSegment;
   }
   gear.append(gear.at(0));
   QRectF rgear = gear.boundingRect();
   qreal ScaleX = Rect.width()/ rgear.width();
   qreal ScaleY = Rect.height()/ rgear.height();
   gear = QTransform::fromScale(ScaleX, ScaleY).map(gear);
   rgear = gear.boundingRect();
   gear = QTransform::fromTranslate(Rect.left()- rgear.left(), Rect.top()- rgear.top()).map(gear);
   polygonList.append(gear);
   return polygonList;
}
...
QList<QPolygonF> ComputePuzzle(QRectF Rect,int Forme/*, bool isRight*/) 
{
   QList<QPolygonF> List;
   QPainterPath Path;
   ...
   qreal RulerX  = Rect.left()+(Rect.width()-(QuarterW/2)-COSSINW)/2;
   qreal RulerY  = Rect.top()+(Rect.height()-(QuarterH/2)-COSSINH)/2;
   ...
   QRectF NewRect;
   //if( isRight )
   //   NewRect = QRectF(Rect.left(), Rect.top(), (Forme & OPTION_RIGHT)==OPTION_RIGHT ? Rect.width()-QuarterW+COSSINiW : Rect.width(), Rect.height()-(QuarterH/2)-COSSINH);
   //else
      NewRect   = QRectF(Rect.left(), Rect.top(), Rect.width()-QuarterW+COSSINiW, Rect.height()-(QuarterH/2)-COSSINH);
   QRectF RectUp    = QRectF(RulerX-QuarterW/2,       NewRect.top()-COSSINiH,    QuarterW, QuarterH);
   QRectF RectDown  = QRectF(RulerX-QuarterW/2,       NewRect.bottom()-COSSINiH, QuarterW, QuarterH);
   QRectF RectLeft  = QRectF(NewRect.left()-COSSINiW,  RulerY-QuarterH/2,        QuarterW, QuarterH);
   QRectF RectRight = QRectF(NewRect.right()-COSSINiW, RulerY-QuarterH/2,        QuarterW, QuarterH);
   ...
   {
      Path.lineTo(QPointF(RectRight.left()+COSSINiW,RulerY-COSSINH));
...
{
   return ComputePolygon(BackgroundForm, QRectF(left, top, width, height));
}

QList<QPolygonF> ComputePolygon(int BackgroundForm, QRectF r)
{
   qreal CenterX = r.left() + r.width() / 2;
   qreal CenterY = r.top() + r.height() / 2;
   qreal RayX = 0;
   qreal RayY = 0;
    ...
    switch (BackgroundForm) 
    {
        case SHAPEFORM_ROUNDRECT : // Round rect
            RayX = qBound(2.0, r.width()/10.0, 16.0);//     if (RayX > 16) RayX = 16; else if (RayX < 2)  RayX = 2;
            RayY = qBound(2.0, r.height()/10, 16.0);//    if (RayY>16) RayY=16; else if (RayY<2)  RayY=2;
            return ComputePolygonRoundRect(r,RayX,RayY);
            break;
        case SHAPEFORM_BUBBLE : // Bubble
            RayX = 2*r.width()/10;   if (RayX < 4)  RayX = 4;
            RayY = 2*r.height()/10;  if (RayY < 4)  RayY = 4;
            return ComputePolygonRoundRect(r,RayX,RayY);
            break;
        case SHAPEFORM_ELLIPSE          : return ComputePolygonEllipse(r);                                          break;  // Ellipse
        case SHAPEFORM_TRIANGLEUP       : return ComputePolygonR(r.width(),r.height(),CenterX,CenterY,3,90);                break;  // Triangle Up
        case SHAPEFORM_TRIANGLERIGHT    : return ComputePolygonR(r.width(),r.height(),CenterX,CenterY,3,0);                 break;  // Triangle Right
        case SHAPEFORM_TRIANGLEDOWN     : return ComputePolygonR(r.width(),r.height(),CenterX,CenterY,3,-90);               break;  // Triangle Down
        case SHAPEFORM_TRIANGLELEFT     : return ComputePolygonR(r.width(),r.height(),CenterX,CenterY,3,-180);              break;  // Triangle left
        case SHAPEFORM_RIGHTTRIANGLEUL  : return ComputeRightTriangle(r,0);                                         break;  // Right triangle Up-Left
        case SHAPEFORM_RIGHTTRIANGLEUR  : return ComputeRightTriangle(r,1);                                         break;  // Right triangle Up-Right
        case SHAPEFORM_RIGHTTRIANGLEDL  : return ComputeRightTriangle(r,2);                                         break;  // Right triangle Down-Left
        case SHAPEFORM_RIGHTTRIANGLEDR  : return ComputeRightTriangle(r,3);                                         break;  // Right triangle Down-Right
        case SHAPEFORM_RHOMBUS          : return ComputePolygonR(r.width(),r.height(),CenterX,CenterY,4,0);                 break;  // Losange
        case SHAPEFORM_PENTAGON         : return ComputePolygonR(r.width(),r.height(),CenterX,CenterY,5,90-(double(360)/5));break;  // Pentagone
        case SHAPEFORM_HEXAGON          : return ComputePolygonR(r.width(),r.height(),CenterX,CenterY,6,-(double(360)/6));  break;  // Hexagone
        case SHAPEFORM_OCTOGON          : return ComputePolygonR(r.width(),r.height(),CenterX,CenterY,8,-(double(360)/8));  break;  // Octogone
        case SHAPEFORM_SIMPLEARROWUP    : return ComputeSimpleArrow(r,0);                                           break;  // Simple arrow top
        case SHAPEFORM_SIMPLEARROWRIGHT : return ComputeSimpleArrow(r,-90);                                         break;  // Simple arrow right
        case SHAPEFORM_SIMPLEARROWDOWN  : return ComputeSimpleArrow(r,180);                                         break;  // Simple arrow down
        case SHAPEFORM_SIMPLEARROWLEFT  : return ComputeSimpleArrow(r,90);                                          break;  // Simple arrow left
        case SHAPEFORM_DOUBLEARROWVERT  : return ComputeDoubleArrow(r,0);                                           break;  // Double vert arrows
        case SHAPEFORM_DOUBLEARROWHORIZ : return ComputeDoubleArrow(r,90);                                          break;  // Double horiz arrows
        case SHAPEFORM_DOUBLEARROWDIAG1 : return ComputeDoubleArrow(r,-45);                                         break;  // Double diag arrows 1
        case SHAPEFORM_DOUBLEARROWDIAG2 : return ComputeDoubleArrow(r,45);                                          break;  // Double diag arrows 2
        case SHAPEFORM_STRIANGLEUP      : return ComputeSingleTriangle(r,0);                                        break;  // Single triangle top
        case SHAPEFORM_STRIANGLERIGHT   : return ComputeSingleTriangle(r,-90);                                      break;  // Single triangle right
        case SHAPEFORM_STRIANGLEDOWN    : return ComputeSingleTriangle(r,180);                                      break;  // Single triangle down
        case SHAPEFORM_STRIANGLELEFT    : return ComputeSingleTriangle(r,90);                                       break;  // Single triangle left
        case SHAPEFORM_DTRIANGLEUP      : return ComputeDoubleTriangle(r,0);                                        break;  // Double chevron top
        case SHAPEFORM_DTRIANGLERIGHT   : return ComputeDoubleTriangle(r,-90);                                      break;  // Double chevron right
        case SHAPEFORM_DTRIANGLEDOWN    : return ComputeDoubleTriangle(r,180);                                      break;  // Double chevron down
        case SHAPEFORM_DTRIANGLELEFT    : return ComputeDoubleTriangle(r,90);                                       break;  // Double chevron left
        case SHAPEFORM_SCHEVRONUP       : return ComputeSingleChevron(r,0,OPTION_UP|OPTION_DOWN);                   break;  // Single chevron up
        case SHAPEFORM_SCHEVRONRIGHT    : return ComputeSingleChevron(r,-90,OPTION_LEFT|OPTION_RIGHT);              break;  // Single chevron right
        case SHAPEFORM_SCHEVRONDOWN     : return ComputeSingleChevron(r,180,OPTION_UP|OPTION_DOWN);                 break;  // Single chevron down
        case SHAPEFORM_SCHEVRONLEFT     : return ComputeSingleChevron(r,90,OPTION_LEFT|OPTION_RIGHT);               break;  // Single chevron left
        case SHAPEFORM_PUSHEDUP         : return ComputeSingleChevron(r,0,OPTION_DOWN);                             break;  // Pushed up
        case SHAPEFORM_PUSHEDRIGHT      : return ComputeSingleChevron(r,-90,OPTION_LEFT);                           break;  // Pushed right
        case SHAPEFORM_PUSHEDDOWN       : return ComputeSingleChevron(r,180,OPTION_UP);                             break;  // Pushed down
        case SHAPEFORM_PUSHEDLEFT       : return ComputeSingleChevron(r,90,OPTION_RIGHT);                           break;  // Pushed left
        case SHAPEFORM_SHARPUP          : return ComputeSingleChevron(r,0,OPTION_UP);                               break;  // Sharp up
        case SHAPEFORM_SHARPRIGHT       : return ComputeSingleChevron(r,-90,OPTION_RIGHT);                          break;  // Sharp right
        case SHAPEFORM_SHARPDOWN        : return ComputeSingleChevron(r,180,OPTION_DOWN);                           break;  // Sharp down
        case SHAPEFORM_SHARPLEFT        : return ComputeSingleChevron(r,90,OPTION_LEFT);                            break;  // Sharp left
        case SHAPEFORM_DCHEVRONUP       : return ComputeDoubleChevron(r,0);                                         break;  // Double chevron up
        case SHAPEFORM_DCHEVRONRIGHT    : return ComputeDoubleChevron(r,-90);                                       break;  // Double chevron right
        case SHAPEFORM_DCHEVRONDOWN     : return ComputeDoubleChevron(r,180);                                       break;  // Double chevron down
        case SHAPEFORM_DCHEVRONLEFT     : return ComputeDoubleChevron(r,90);                                        break;  // Double chevron left
        case SHAPEFORM_STAR4            : return ComputeStar(r,4);                                                  break;  // Star 4
        case SHAPEFORM_STAR5            : return ComputeStar(r,5);                                                  break;  // Star 5
        case SHAPEFORM_STAR6            : return ComputeStar(r,6);                                                  break;  // Star 6
        case SHAPEFORM_STAR8            : return ComputeStar(r,8);                                                  break;  // Star 8
        case SHAPEFORM_GEAR6            : return ComputeGear(r,6);                                                  break;  // Gear 6
        case SHAPEFORM_GEAR8            : return ComputeGear(r,8);                                                  break;  // Gear 8
        case SHAPEFORM_GEAR10           : return ComputeGear(r,10);                                                 break;  // Gear 10
        case SHAPEFORM_GEAR12           : return ComputeGear(r,12);                                                 break;  // Gear 12
        case SHAPEFORM_HEART            : return ComputeHeart(r);                                                   break;  // Heart
        case SHAPEFORM_SPADE            : return ComputeSpade(r);                                                   break;  // Spade
        case SHAPEFORM_CLUB             : return ComputeClub(r);                                                    break;  // Club
        case SHAPEFORM_PUZZLEUL         : return ComputePuzzle(r,OPTION_RIGHT|OPTION_DOWN);                         break;  // Puzzle Up-Left
        case SHAPEFORM_PUZZLEUC         : return ComputePuzzle(r,OPTION_LEFT|OPTION_RIGHT|OPTION_DOWN);             break;  // Puzzle Up-Center
        case SHAPEFORM_PUZZLEUR         : return ComputePuzzle(r,OPTION_LEFT|OPTION_DOWN);                          break;  // Puzzle Up-Right
        case SHAPEFORM_PUZZLEML         : return ComputePuzzle(r,OPTION_UP|OPTION_RIGHT|OPTION_DOWN);               break;  // Puzzle Middle-Left
        case SHAPEFORM_PUZZLEMC         : return ComputePuzzle(r,OPTION_UP|OPTION_LEFT|OPTION_RIGHT|OPTION_DOWN);   break;  // Puzzle Middle-Center
        case SHAPEFORM_PUZZLEMR         : return ComputePuzzle(r,OPTION_UP|OPTION_LEFT|OPTION_DOWN);                break;  // Puzzle Middle-Right
        case SHAPEFORM_PUZZLEDL         : return ComputePuzzle(r,OPTION_UP|OPTION_RIGHT);                           break;  // Puzzle Down-Left
        case SHAPEFORM_PUZZLEDC         : return ComputePuzzle(r,OPTION_UP|OPTION_LEFT|OPTION_RIGHT);               break;  // Puzzle Down-Center
        case SHAPEFORM_PUZZLEDR         : return ComputePuzzle(r,OPTION_UP|OPTION_LEFT);                            break;  // Puzzle Down-Right

        //case SHAPEFORM_PUZZLEUR_NEW         : return ComputePuzzle(r,OPTION_LEFT|OPTION_DOWN,true);                 break;  // Puzzle Up-Right --new--
        //case SHAPEFORM_PUZZLEMR_NEW         : return ComputePuzzle(r,OPTION_UP|OPTION_LEFT|OPTION_DOWN,true);       break;  // Puzzle Middle-Right --new--
        //case SHAPEFORM_PUZZLEDR_NEW         : return ComputePuzzle(r,OPTION_UP|OPTION_LEFT,true);                   break;  // Puzzle Down-Right --new--

        case SHAPEFORM_BINOCULARSV      : return ComputeOculars(r,2,OPTION_UP);                                     break;  // Binoculars vertical
        case SHAPEFORM_TRINOCULARSV     : return ComputeOculars(r,3,OPTION_UP);                                     break;  // Trinoculars vertical
        case SHAPEFORM_BINOCULARSH      : return ComputeOculars(r,2,OPTION_LEFT);                                   break;  // Binoculars horizontal
        case SHAPEFORM_TRINOCULARSH     : return ComputeOculars(r,3,OPTION_LEFT);                                   break;  // Trinoculars horizontal
        case SHAPEFORM_FOUROCULARS      : return ComputeOculars(r,4,OPTION_UP);                                     break;  // Four oculars
        case SHAPEFORM_TRAPEZOIDUPLEFT  : return ComputeTrapezoid(r,OPTION_UP,OPTION_LEFT);                         break;  // Trapezoid Up-Left
        case SHAPEFORM_TRAPEZOIDUP      : return ComputeTrapezoid(r,OPTION_UP,OPTION_LEFT|OPTION_RIGHT);            break;  // Trapezoid Up
        case SHAPEFORM_TRAPEZOIDUPRIGHT : return ComputeTrapezoid(r,OPTION_UP,OPTION_RIGHT);                        break;  // Trapezoid Up-Right
        case SHAPEFORM_TRAPEZOIDOWNLEFT : return ComputeTrapezoid(r,OPTION_DOWN,OPTION_LEFT);                       break;  // Trapezoid Down-Left
        case SHAPEFORM_TRAPEZOIDOWN     : return ComputeTrapezoid(r,OPTION_DOWN,OPTION_LEFT|OPTION_RIGHT);          break;  // Trapezoid Down
        case SHAPEFORM_TRAPEZOIDOWNRIGHT: return ComputeTrapezoid(r,OPTION_DOWN,OPTION_RIGHT);                      break;  // Trapezoid Down-Right
        case SHAPEFORM_TRAPEZOIDLEFTUP  : return ComputeTrapezoid(r,OPTION_LEFT,OPTION_UP);                         break;  // Trapezoid Left-Up
        case SHAPEFORM_TRAPEZOIDLEFT    : return ComputeTrapezoid(r,OPTION_LEFT,OPTION_UP|OPTION_DOWN);             break;  // Trapezoid Left
        case SHAPEFORM_TRAPEZOIDLEFTDOWN: return ComputeTrapezoid(r,OPTION_LEFT,OPTION_DOWN);                       break;  // Trapezoid Left-Down
        case SHAPEFORM_TRAPEZOIDRIGHTUP : return ComputeTrapezoid(r,OPTION_RIGHT,OPTION_UP);                        break;  // Trapezoid Right-Up
        case SHAPEFORM_TRAPEZOIDRIGHT   : return ComputeTrapezoid(r,OPTION_RIGHT,OPTION_UP|OPTION_DOWN);            break;  // Trapezoid Right
        case SHAPEFORM_TRAPEZOIDRIGHTDOWN:return ComputeTrapezoid(r,OPTION_RIGHT,OPTION_DOWN);                      break;  // Trapezoid Right-Down
        case SHAPEFORM_PARALLELOLEFT    : return ComputeParallelo(r,OPTION_LEFT);                                   break;  // Parallelogram Left
        case SHAPEFORM_PARALLELORIGHT   : return ComputeParallelo(r,OPTION_RIGHT);                                  break;  // Parallelogram Right
        case SHAPEFORM_PARALLELOUP      : return ComputeParallelo(r,OPTION_UP);                                     break;  // Parallelogram Up
        case SHAPEFORM_PARALLELODOWN    : return ComputeParallelo(r,OPTION_DOWN);                                   break;  // Parallelogram Down
        case SHAPEFORM_KEYHOLE          : return ComputeKeyHole(r);                                                 break;  // KeyHole
        case SHAPEFORM_PLUS             : return ComputePlus(r);                                                    break;  // Plus
        case SHAPEFORM_HOURGLASS        : return ComputeHourglass(r);                                               break;  // Hourglass
        default                         : return ComputePolygonRect(r);                                             break;  // No shape or rectangle or unknown shape
    }
}
...
QRectF PolygonToRectF(QList<QPolygonF> Polygon) 
{
   ...
   if ((Polygon.count() == 0) || (Polygon.at(0).toList().count() == 0)) 
         return QRectF();
...
bool getShapeRatio(int shape, double *ratioX, double *ratioY)
{
   if( shape >= 0 && shape < NBR_SHAPEFORM )
   {
      cShapeFormDefinition *pShape = &ShapeFormDefinition[shape];
      if( pShape ->RatioX == -1.0 )
      {
         QRectF tmpRect = PolygonToRectF(ComputePolygon(shape, QRectF(0,0,100,100)));
         pShape->RatioX = 100.0 / tmpRect.width();
         pShape->RatioY = 100.0 / tmpRect.height();
      }
      *ratioX = pShape->RatioX;
      *ratioY = pShape->RatioY;
      return true;
   }
   *ratioX = 1.0;
   *ratioY = 1.0;

...
void cCGrdOrientationComboBoxItem::paint(QPainter *painter,const QStyleOptionViewItem &option,const QModelIndex &index) const {
    ...
    if ((ColorNum>=0)&&(ColorNum<MAXGRADIENTORIENTATION)) {
        ...
        if (ComboBox->Brush->BrushType==BRUSHTYPE_GRADIENT3) 
           Gradient.setColorAt(ComboBox->Brush->Intermediate,QColor(ComboBox->Brush->ColorIntermed));
        painter->setBrush(Gradient);
        painter->drawRect(option.rect);
        if (ColorNum != GRADIENTORIENTATION_RADIAL)
        {
           QLinearGradient *lgrad = (QLinearGradient *)&Gradient;
//           qDebug() << "grad start " << lgrad->start() << " grad end " << lgrad->finalStop() << " rect is " << option.rect;
        }
    } else {
...
void cCGrdOrientationComboBox::SetCurrentBrush(cBaseBrushDefinition *TheBrush) 
{
    ...
    //QModelIndex mi = ((QTableWidget *)view())->model()->index(Brush->GradientOrientation/3, Brush->GradientOrientation - (Brush->GradientOrientation / 3) * 3);
    //((QTableWidget *)view())->setCurrentIndex(mi);
    setCurrentIndex(Brush->GradientOrientation/3);
    setModelColumn(Brush->GradientOrientation - (Brush->GradientOrientation / 3) * 3);
    ((QTableWidget *)view())->setCurrentCell(Brush->GradientOrientation/3,Brush->GradientOrientation-(Brush->GradientOrientation/3)*3, QItemSelectionModel::SelectCurrent);
    MakeIcons();
}
...
{
    STOPMAJ=true;
    int index = ((QTableWidget *)view())->currentRow() * 3 + ((QTableWidget *)view())->currentColumn();
    setCurrentIndex(((QTableWidget *)view())->currentRow());
    MakeIcons();
    emit currentIndexChanged(index/*((QTableWidget *)view())->currentRow()*3+((QTableWidget *)view())->currentColumn()*/);
    STOPMAJ=false;
}

...
void FltBrightness(QImage &Img,qint32 bn) 
{
   ...
   {
      bits = (quint8*)Img.bits() + Img.width()*y*sizeof(rgba);
...
// Adapt From fmt_filters<http://ksquirrel.sourceforge.net/subprojects.php>
void FltContrastNew(QImage &Img,qint32 contrast);
...
void FltGamma(QImage &Img,double L) {
    ...
    for (qint32 x=1;x<256;++x)	GT[x]=(quint8)floor(255*pow((double)x/255.0,1.0/L));
...
void FltColorize(QImage &Img,qint32 red,qint32 green,qint32 blue) 
{
   ...
   for (qint32 y = 0; y < Img.height(); y++) 
   {
      ...
      for (qint32 x = 0; x < Img.width(); x++) 
      {
         for (qint32 v = 0; v < 3; ++v) 
         {
            val = (qint32)*(bits+v)+V[v];
            if (val > 255) 
               *(bits+v) = 255;
            else if (val < 0)
               *(bits+v) = 0;
